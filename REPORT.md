...
| 37 | The Windows curl-versus-httpx comparison, as the diagnosis of why a LINUX production fetcher is refused | **It eliminated four causes correctly and then named a fifth that does not exist on the platform that matters.** The UA, the header set, the HTTP version and the client library were each ruled out by holding everything else constant, and that work was sound. What was left standing was the TLS stack, because the curl that succeeded linked Schannel and Python links OpenSSL. The inference was that OpenSSL's ClientHello is what gets refused. **The confound was never controlled: every 200 came from a residential Windows address and every 403 from the same machine's Python, so the source address was held constant only within the laptop.** From `ubuntu-latest` both clients fail 14/14. The variable that looked like TLS was reputation. | **By running the probe that was written specifically because this could not be extrapolated, and only because it was run BEFORE the fix it would have justified.** The fix was one approval away: swap the transport to curl, or reach for `curl_cffi`. Every one of those options is aimed at a client that is not the problem, and each would have shipped, passed its own tests, and changed nothing, while the report carried a confident and wrong account of why. **The general form: eliminating N causes does not license the N+1th. A residual hypothesis inherits all the confounds the elimination did not control, and "what is left after I ruled things out" feels like a measurement while being an inference.** The narrower form, which this project has now hit twice: a measurement taken on a developer laptop about a system that runs on Linux is a different system's measurement. See entry 31, where the same laptop's fetcher was the probe. |
...
| **NEW, 19 September: the scheduled refresh reports success daily while doing nothing, and that is why nothing has refreshed** | **OPEN, not fixed, and this is the actual cause of the twelve-day staleness.** Fourteen scheduled runs, one a day through today, all green, all 24 to 37 seconds. None attempted a fetch: `DATABASE_URL` is unset, so `recrawl.yml` sets `mode=dry-run`, the no-op step exits 0, `Re-crawl (live)` is skipped on its `if:`, and the missing artifact is ignored by `if-no-files-found: ignore`. 14 pages at a 2.0s delay cannot finish in 24s; those seconds are the pip install. The skip itself is correct and deliberate (Phase 8 has no hosted database). **The defect is that a job doing nothing and a job finding nothing changed are both a green check at 30 seconds**, which is the `sources` table's failed-vs-unchanged ambiguity one layer up. **Option D is now BUILT** in `recrawl.yml`: the skip step emits a `::warning title=Scheduled refresh did NOT run::` annotation and a summary block reading "Sources fetched: **0 of 14**", verified by executing the step against a temporary `GITHUB_STEP_SUMMARY`. It still exits 0 deliberately, because the skip is currently correct; option A (fail the run) lands with the `DATABASE_URL` secret. **Sequencing note, 19 September, REVISED the same day: still do not set that secret, but NOT for the reason first given.** The original note said the first live run would die on `_diff_node`'s opening SELECT and mark all 14 sources `fetch_failed` without fetching anything, which made the skip the only thing preventing a corpus-wide corruption of the health state. **That is no longer true.** The schema divergence is closed (see its row above: `check_schema` reports no missing objects against production), so the first live run would reach the fetcher instead of dying before it. Two different blockers now stand in front of the secret: the TLS block, which would produce fourteen GENUINE fetch failures rather than fourteen spurious ones, and the unimplemented `HostedEmbedder` seam, which means a meaningful change could be detected and then not re-indexed. The skip is still correct. **The corpus-corruption argument for it is retired, and the record should not carry a retired reason as if it were current.** **Option E, the staleness watchdog, is approved and planned but NOT built** (see "Option E in detail": a separate scheduled workflow first, Grafana Cloud alerting as the better long-term home, alert at `age_hours > 48`, escalate at 7 days). **Corrects an earlier row that blamed the 403 for the staleness.** |
| ~~NEW, 19 September: production's schema is frozen pre-ADR-0014, and the first re-crawl would mark all 14 sources fetch_failed without fetching anything~~ **RESOLVED, 19 September** | **CLOSED. `python -m app.check_schema` ran against production and reported no missing objects: 6 tables, 33 columns, one extra (`playing_with_neon`, Neon's default demo table, correctly non-fatal). The user ran it; the figures in this row are theirs.** The gap was closed from the CODE side, not the database side: `sources.rule_effective_date` does not appear in that output as an extra either, because production never had it and `infra/sql/init.sql` no longer declares it (Option B, ADR 0022). Nothing was migrated, no DDL was run against Neon, and `sources.last_indexed_body` was already added and backfilled in step 2. **Setting `DATABASE_URL` is no longer the corpus-corrupting move this row described**: `_diff_node`'s opening SELECT no longer names a column production lacks, so the first live run would not mark 14 sources `fetch_failed` without fetching. It is still not SAFE to set, for two different reasons now standing in front of it: the TLS block in the row below, and the unimplemented `HostedEmbedder` seam (`recrawl.yml` says so in its own comment) which means a meaningful change could be detected and then not re-indexed. **The reason to wait has changed and the record should say which.** Original entry follows. Confirmed against production's `information_schema`: `sources.rule_effective_date` and `sources.last_indexed_body` are ABSENT; `documents.rule_effective_date` and `documents.content_tsv` are present. Production is post-Phase-5, pre-ADR-0014. `infra/sql/init.sql` only ever reaches the local compose postgres via `docker-entrypoint-initdb.d` (which runs only on an empty data dir); there is no migration runner and nothing applies it to Neon, so the hosted schema froze at whatever was applied by hand. **The consequence is worse than a failed run.** `_diff_node`'s first statement selects both missing columns, and `touch_last_verified`/`_embed_and_store` both write one, so every success path is dead; but `record_source_failure` touches only original-shape columns and SUCCEEDS. The first run would write `status='fetch_failed'`, bump `consecutive_failures`, and store the `UndefinedColumn` text in `last_error` for **all 14 sources, having fetched nothing** -- a corpus-wide health state that is specifically and confidently wrong. **It also explains the 7 September timestamp**: no re-crawl has ever run against production. That `last_verified_at` was written by an ingest, from a code version predating ADR 0014, since today's `_embed_and_store` would fail on this schema too. Surfaced by `sync_annotations` failing with a raw `UndefinedColumn`; see instrument table entry 33. |
| ~~NEW, 19 September: the fetcher is blocked at the TLS layer~~ **DIAGNOSIS RETIRED, 19 September, by the probe. The BLOCK is real; the CAUSE was wrong.** | **CORRECTED. `fetch-probe.yml` ran on `ubuntu-latest` and returned world three: httpx 0/14 AND curl 0/14, plus 403 on all three `robots.txt` files. Not the client. The TLS-fingerprint hypothesis is dead**, and with it every client-side fix option: `curl_cffi`, `tls-client`, a browser UA, switching the transport to curl. None of them addresses a block that refuses curl too. They are retired, not deferred, and must not be reached for again without new evidence. What the Windows numbers actually measured was one laptop's Schannel curl getting through a WAF that refuses OpenSSL clients from an Azure range; the variable that looked like TLS was confounded with the source address. **This killed a fix that was one approval away from being built.** See instrument entry 37 and the probe section below. The consequence for the scheduled refresh is NOT sequencing, it is the HOST: see the row below. **Original entry follows.** **OPEN, not fixed, and now FIRST in the queue.** The sequencing on this row has moved twice in one day and both moves are recorded rather than overwritten. It was first; then the schema finding displaced it; now the schema finding is CLOSED (see the row above), so this is first again, alongside the unimplemented `HostedEmbedder` seam. **The defect itself has not changed at any point and none of this is new evidence about it.** What follows is the original text, including the retired sequencing claim in the paragraph after the header, which is left visible because a reader who sees only the current verdict cannot tell that the ordering was wrong twice. **Original entry follows.** Setting `DATABASE_URL` was next on the list and would not have reached a fetch at all: the re-crawl dies on `_diff_node`'s opening SELECT, before the fetcher is ever called, and would have reported the result as fourteen fetch failures -- pointing straight back at this TLS block, which had nothing to do with it. Fix the schema question first; this one is latent behind it, not in front of it.**Original entry follows.** `app/ingest.py::fetch_page` (httpx) returns 403 on all 14 sources including `ice.gov/robots.txt`. Never once reached in CI, because CI has never got past the gate above. Four causes eliminated: not the UA (httpx with no UA still 403s; curl with the project's exact bot UA is served), not the headers (httpx trimmed byte-identical to curl still 403s), not the HTTP version (the installed curl has no HTTP/2 at all, so every 200 was HTTP/1.1, same as httpx), not the client code (stdlib urllib fails identically). What remains is the TLS fingerprint: Python links OpenSSL 3.0.18, the curl that works uses Schannel. **All 14 URLs are explicitly ALLOWED by each host's live robots.txt under the project's real UA and parser, 14 allowed / 0 denied**, so a permitted, honestly-identified, rate-limited crawler is being refused by a bot-detection layer that never reads that permission. **No honest fix exists at the HTTP layer and none was applied**: every remaining lever (`curl_cffi impersonate=`, `tls-client`, a browser UA) works by forging the client's identity, which is outside this project's crawl policy and was refused. **The measurement that must come first has not been taken**: every number here is from Windows/Schannel, a TLS stack that does not exist on Linux, where CI and production run and where curl links the same OpenSSL Python does. See "The 19 September re-crawl" below. |
...
| **NEW, 19 September: production crawls with a User-Agent that has no contact address and a repository URL that does not resolve** | **OPEN, not fixed, and it undercuts an argument this report makes twice.** Measured on the production machine by the Fly probe, which prints the UA it is about to send: production sends `OfficeHoursBot/0.1 (+https://github.com/office-hours)`. **No contact address, and `github.com/office-hours` is not this project's repository.** There are FOUR different literals in the tree and the one that wins in production is the only one with neither a contact nor a real URL: `app/config.py:210` (the default, what production sends), `.env.example:201` and the gitignored local `.env:116` (both the full form with the real repo and `patel.rishabh@northeastern.edu`), and `docker-compose.yml:55` (`contact: set-a-real-contact-here`). Fly's `[env]` block sets 19 keys and USER_AGENT is not among them; no secret sets it either, or the probe would have printed it; and `.env` is not copied into the image, so `env_file=".env"` resolves to nothing under `WORKDIR /app`. **The default wins in every environment except a local run from the repo root.** This matters beyond tidiness: the robots-versus-WAF finding and the refusal to use a fingerprint-forging library both rest on this crawler being honestly identified and reachable, and the reachable half was never being sent. See instrument entry 39. |
| **NEW, 19 September: `RobotsCache` fails OPEN, so an unreadable robots.txt reads as permission** | **OPEN, not fixed, deliberately, and recorded separately from the row below because it is a defect in this project's code rather than a fact about the sites.** `app/ingest.py::RobotsCache._get_parser` maps any status >= 400, and any `httpx.HTTPError`, to `parser.parse([])`, which permits everything. **This is a fail-open in the single place the crawl policy is enforced.** It also reverses the stdlib: measured, `urllib.robotparser.RobotFileParser.read()` sets `disallow_all = True` on 401/403; this class bypasses `.read()` and turns that block into a permit. Harmless today only by coincidence, since the page fetch fails too; the moment a page fetch succeeds where the robots fetch does not, the crawler crawls on an assumption. Correct behaviour is three-way, not two-way: 401/403 DISALLOW, 404 and other 4xx ALLOW (genuinely no robots.txt, and blocking here would wrongly stop the crawler on every site that publishes none), 5xx and timeouts and connection errors DISALLOW (RFC 9309 calls this unreachable and specifies full disallow; today it permits). See instrument entry 38. |
| **NEW, 19 September: the sites' own robots.txt permits every URL the WAF refuses, AND from a GitHub runner the WAF refuses robots.txt itself** | **SHARPENED, 19 September, by the probe. The Linux measurement returned, and it makes this worse rather than settling it.** From `ubuntu-latest` all three `robots.txt` files returned 403. **The WAF blocks the request before serving the file that grants the permission**, so a crawler cannot read the policy it is trying to honour. The 14-allowed/0-denied result below was measured from Windows, where the file was readable; from the host the cron actually runs on, it is not. A latent consequence worth naming: `app/ingest.py::RobotsCache._get_parser` treats any status >= 400 as an empty ruleset (`parser.parse([])`), and an empty ruleset allows everything, so a WAF-blocked robots.txt silently becomes "allowed". Harmless today only because nothing gets fetched either way. **Original entry follows.** **Recorded, not actionable on its own, and it stands whatever the Linux measurement returns.** All 14 URLs tested against each host's live `robots.txt` with the project's own `RobotFileParser` and its real UA: **14 allowed, 0 denied.** The single `Disallow: /` on `studyinthestates.dhs.gov` and `ice.gov` belongs to a `User-agent: PetalBot` block, not to `*`. A crawler these sites explicitly permit, identifying honestly with a contact address and honouring an unrequested two-second delay, is refused by a bot-detection layer that never reads that permission. **This is what makes the disguise question a dilemma rather than an obvious no**, and it is also why it still resolves to no: a published robots.txt is permission to crawl, not consent to be deceived about who is crawling. |
| ~~NEW, 19 September: two hand-run tools that write to production have NO tests, and their only verification was thrown away~~ **CLOSED, same day** | **FIXED.** `tests/test_sync_annotations.py` (16 tests) and `tests/test_check_schema.py` (15) promoted from the throwaway harnesses, plus 3 statement-level tests added to `tests/test_backfill_source_bodies.py`. Suite 505 -> 539. **All 34 run with NO DATABASE**, against fake connections that record every statement and transaction boundary, so they are the only tests in this project executable on the Windows machine that holds the snapshots -- stated at the top of each file rather than left to be discovered. The three findings the harnesses made are kept as REGRESSIONS, not scenarios: the never-written column list for `sync_annotations` (the `_SYNCED_ANNOTATIONS` decorative-constant defect), the headline-matches-exit-code invariant for `check_schema` (which regressed as `schema ok` printed on a run exiting 1, now parametrised over all three ways a drift can be fatal), and the proof that `backfill_source_bodies` no longer writes `rule_effective_date`. Each forbidden-column check ships with a CONTROL asserting the predicate trips on the pre-fix statement, because an absence check pointed at nothing passes forever. Each file states what a fake connection cannot establish: `rowcount` semantics and the real commit boundary are stubbed, measured for the backfill by its production run and still unobserved for the other two. **Original entry follows.** **OPEN, not fixed.** `app/sync_annotations.py` and `app/check_schema.py` have no test file and are imported by no test; `grep` across `tests/` returns nothing for either. (`app/recrawl.py` and `app/ingest.py` ARE covered, under differently-named files -- `test_freshness.py` and `test_chunking.py` -- so this is a gap of two modules, not five.) `sync_annotations` is the worse of the two: it writes `sources` AND `documents` in production, and exists specifically to be reached for when a legally-consequential annotation has to change quickly. **The sharper problem is what it was verified WITH.** Both were checked against fake-connection harnesses that recorded every statement and transaction boundary, drove five and six scenarios respectively, and caught two real defects (a decorative constant, and a `schema ok` headline printed on a run that exits 1). Those harnesses live in a session scratchpad, not in the repository. **They did real work and then evaporated**, which is the same shape as the diff baseline existing only in a gitignored directory: the artifact that establishes correctness is not kept anywhere the project can find it again. Promoting them to `tests/test_sync_annotations.py` and `tests/test_check_schema.py` is mostly mechanical, since the fakes are already written and already pass. Not done. |
| ~~NEW, 19 September: the Linux measurement that must precede any fetcher fix~~ **DONE. It reported world three and the file is deleted.** | **REPORTED AND CLOSED, 19 September. Result: httpx 0/14, curl 0/14, and 403 on all three `robots.txt` files. The third world, address reputation, so the client is not the problem and every client-side fix is retired** (see the TLS row above and "The probe reported world three" below). The file has been DELETED, as its own header instructed, so this row is the only remaining record of what it did. It ran once by hand and reported; keeping a throwaway diagnostic on disk after that is how it becomes something someone maintains. **Original entry follows.** `.github/workflows/fetch-probe.yml` is written and on disk; it has not been run, because running it requires a push and that is yours. One throwaway `workflow_dispatch` job, no secret, no database, no writes: it fetches all 14 manifest URLs from `ubuntu-latest` with both httpx and curl, plus each host's robots.txt, and prints a per-source and per-host table with an auto-computed verdict. It distinguishes the only three outcomes that matter: httpx 200 means there is no bug outside one Windows laptop and the whole finding collapses to a note; httpx 403 with curl 200 means a real client problem that reproduces where CI runs; both 403 means address reputation (Actions runners are Azure-ranged) and a client-side fix would be aimed at the wrong thing. **No fetcher fix is recommended until this lands.** Delete the file once it has reported. |
...
**Nothing goes red. CORRECTED 19 September 2026: this was false when it was written.** Eleven
`qualify_future_dated_figures` call sites in the test suite pin `today` to a literal, so those eleven
stay green, and the twelfth call site counted here was never a test at all but `app/pipeline.py`'s
own. What the count missed is a twelfth PATH:
`test_temporal_guard_block_path_returns_blocked_unverified_end_to_end` reaches the guard through
`answer_question`, which reads the real clock at `pipeline.py:690` with no seam to pin, and it began
failing on 15 September, the day this paragraph was written, surfacing in CI on the 19th. **The suite
did go red.** This report asserted otherwise from a count of pinned call sites, which is instrument
table entry 30's second failure. The test has since been split: a contract half, green, with a
relative date on the fixture, and a real-rule half that stays red on purpose until the decision below
is made. The re-crawl reasoning that follows is unaffected and still stands. The daily re-crawl does not help either: if DHS leaves
...
### FINDING, HIGH: the fetcher is blocked at the TLS layer (DIAGNOSIS RETIRED: see the probe result below)

> **RETIRED, 19 September 2026, by measurement. Read this section as the record of a wrong
> diagnosis, not as a live finding.** The probe returned world three: from `ubuntu-latest`, httpx
> failed 14/14 AND curl failed 14/14. The block is real and unchanged. The CAUSE named throughout
> this section, the TLS fingerprint, does not exist on the platform that matters. Everything below
> about eliminating the UA, the headers, the HTTP version and the client library still stands as
> measurement; the inference drawn from what remained does not. See "The probe reported world
> three" below.
...
> **RESOLVED, 19 September 2026, the same day, and the fix was in the code rather than in the
> database.** `python -m app.check_schema` against production returns no missing objects: 6 tables,
> 33 columns, one extra (`playing_with_neon`, Neon's default demo table, correctly non-fatal). Run by
> the user. **`sources.rule_effective_date` is not in that output as an extra either**, because
> production never had it and `infra/sql/init.sql` stopped declaring it under Option B (ADR 0022).
> That is the whole resolution: the divergence was closed by deleting the column from the
> declaration, not by migrating Neon. No DDL was run against production for it, and the option this
> section refuses below (making `_diff_node` tolerant of the missing column) was never taken. The
> consequence described in this section, fourteen sources marked `fetch_failed` without a page being
> fetched, can no longer happen, because no success-path statement names a column production lacks.
> **Read the rest of this section as the record of a closed finding**, kept in full because the
> reasoning about why tolerance was refused is the part worth keeping.
...
**MEASURED, 19 September 2026. This was the check's first real execution, and it worked.** Run by the
user, not by the agent. `read_schema`'s `information_schema` SELECT had never touched a database
before this and ran correctly against BOTH ends of the comparison: a pg16 reference with `init.sql`
applied, and Neon. The verdict was clean: **6 tables, 33 columns, no missing objects**, with one extra
(`playing_with_neon`, Neon's default demo table) reported and correctly treated as non-fatal, which is
the asymmetry above doing exactly what it was designed to do on its first outing rather than on a
contrived case. The `6/6 correct` figure above was six cases against `compare`/`render` with
hand-built inputs; this is the first time the reading half of the tool was exercised at all.
...
- ~~**`infra/sql/init.sql` has not been applied to a Postgres.**~~ **MEASURED, 19 September, by the
  user.** `check_schema` applied the edited file to a `pgvector/pgvector:pg16` reference and read its
  `information_schema` back, so the `ALTER TABLE sources` is valid SQL after the clause removal and
  not merely re-read by hand. Comparing that reference against production returned **6 tables, 33
  columns, no missing objects**, one non-fatal extra (`playing_with_neon`).
  `sources.rule_effective_date` appears on neither side, which is the direct evidence that removing
  the declaration closed the divergence rather than papering over it.
...
## The probe reported world three: not the client, and the refresh is on the wrong host

Run by the user on `ubuntu-latest`, 19 September 2026. `fetch-probe.yml` distinguished three worlds
and returned the third:

    httpx  0 / 14
    curl   0 / 14
    robots.txt  403 on all 3 hosts

Both clients refused on every source. **The TLS-fingerprint hypothesis is dead.** The four causes
eliminated on Windows were eliminated correctly; the fifth, the one left standing, does not exist on
the platform that matters. Instrument entry 37 has the general form, and the short version is that
every 200 in the Windows table came from a residential address and every 403 from the same laptop's
Python, so the source address was held constant only within that machine. What looked like TLS was
reputation.

**Every client-side fix option is retired, not deferred.** `curl_cffi` with `impersonate=`,
`tls-client`, a browser User-Agent, switching the transport to curl: each of them changes what the
client looks like, and the block refuses curl too. None addresses the cause. They should not be
reached for again without new evidence, and the ethical objection to them is now beside the point,
because they also do not work.

**The probe killed a fix that was one approval away.** Switching the transport to curl was the
obvious next step, it would have been built, it would have passed its own tests, and it would have
changed nothing, while this report carried a confident and specific account of why the fetcher was
blocked that happened to be wrong. The only reason it did not happen is that the probe was written
and run before the fix it would have justified, on the explicit grounds that a Windows measurement
could not be extrapolated to Linux.

**The robots.txt result is the sharpest thing in the run.** All three `robots.txt` files returned
403. The WAF blocks the request before serving the file that grants the permission, so a crawler
cannot read the policy it is trying to honour. The earlier finding that all 14 URLs are explicitly
allowed, 14 allowed and 0 denied, was measured from Windows where the file was readable. From the
host the cron actually runs on, it is not. Both facts are true at once and belong together: these
sites publish a policy permitting this crawler, and refuse to serve that policy to it.

A latent behaviour falls out of that. `app/ingest.py::RobotsCache._get_parser` treats any status of
400 or above as an empty ruleset, and an empty ruleset allows everything, so a WAF-blocked
robots.txt silently reads as "allowed". Harmless today only because nothing gets fetched either way.

### The consequence for the scheduled refresh: this is not sequencing, it is the host

The cron runs on a GitHub-hosted runner. A GitHub-hosted runner cannot fetch these pages. So setting
`DATABASE_URL` would produce a refresh that fails on all 14 sources, regardless of the
`HostedEmbedder` question and regardless of the schema work that preceded it.

That retires the framing this report has used three times. It was never a question of which blocker
came first. **The job is on a host that cannot do the job.**

**MEASURED, 19 September 2026, and the assumption held.** Run by the user from the orchestrator
machine (`fly_app=oh-orchestrator-rp`, `fly_region=iad`, confirmed by the probe's own
self-identifying line, which exists because an earlier draft printed a verdict about Fly from a
Windows laptop):

    14 / 14 pages 200
    robots.txt 200 on all three hosts

**The GitHub runner's address was the whole problem.** Options A and C are live rather than
untested, and this also settles the 7 September question: the corpus was ingestable from a Fly
address because a Fly address is not blocked. The original text of this paragraph follows, because
the reasoning for demanding the measurement before building on it is the part worth keeping.
**Fly can fetch these pages was an assumption, not a result.** The nearest evidence is that the corpus was ingested on
7 September through this same httpx fetcher, so it worked from somewhere on that date, but where it
ran is not recorded anywhere in this repository, and a competing explanation fits the same facts
equally well: the WAF rule may simply postdate the ingest. Given that this section exists because a
confident diagnosis from the wrong environment survived four rounds of elimination, that assumption
should be measured before anything is built on it.

### What a Fly-hosted refresh would look like, and what it costs

Stated as options with their costs, none of them built.

**A. A separate scheduled Fly Machine, from its own image.** The refresh runs where the orchestrator
already runs, from an image built as the orchestrator image plus the `[freshness]` extra. This is the
option that resolves the most at once: it puts the fetch on a Fly address, and it also answers the
`HostedEmbedder` question without implementing it, because the production image already bakes in the
GGUF model, so the refresh would embed in-process exactly as serving does. Costs: a second image to
build and keep in step, a second machine, and a scheduling mechanism that is not GitHub Actions, so
the run history and the artifact upload that `recrawl.yml` currently gives for free have to be
rebuilt or given up.

**B. Run it inside the existing serving machine.** Cheapest in infrastructure and the one to name
explicitly so it is rejected on the record rather than reached for under time pressure. It requires
adding langgraph to the serving image, which breaks an isolation guarantee that is deliberate,
documented in `app/recrawl.py`'s module docstring, and enforced by its own tests: the production
image is built with `INSTALL_EXTRAS=gguf` precisely so that langchain and langgraph stay out of the
image that answers `/query`. Paying for a refresh with the serving image's dependency surface is a
bad trade.

**C. Keep GitHub Actions as the scheduler and have it call Fly to do the fetching.** The runner
triggers, the Fly container fetches. Keeps the existing run history, the artifact, and the
golden-impact gate. Costs: an authenticated admin endpoint on a service that is currently private
behind the gateway and does one thing, plus a long-running request or a job-status protocol, since
14 pages at a 2 second delay does not fit a normal request timeout.

**D. A self-hosted runner on a permitted network.** Keeps the whole existing workflow unchanged and
moves only the address. Costs a machine to own, patch and secure, which is the largest ongoing
commitment here for the narrowest gain.

**E. Do not move the refresh. Make the staleness visible and accept a frozen corpus.** This is the
honest null option and it is not nothing: the corpus has not refreshed since 7 September, four
separate signals were correct about that throughout, and nothing consumed them. Option E from
"Making a skip visible" is the watchdog that surfaces it. This does not fix the fetch; it stops the
system reporting health it does not have, which is the failure mode this report keeps finding.
...
- `.github/workflows/fetch-probe.yml`, new, `workflow_dispatch` only, throwaway. Reads no secret,
  ...
  **It has since reported (world three) and been deleted, 19 September 2026. This bullet describes
  what that session added, not what is on disk now.**

**Third change, later still: the temporal test was split.** `services/orchestrator/tests/test_guardrails.py`
is the only source file this session touched. CI went red on
`test_temporal_guard_block_path_returns_blocked_unverified_end_to_end`, which is instrument table
entry 30 arriving in the suite. The test was **split, not pinned, not xfailed, not marked, not
relaxed**:

- The contract half keeps the original name and now builds its fixture with `_relative_future_date()`
  instead of the decayed `_FUTURE_DATE` constant. This moves the CHUNK's date, never `today`; the
  pipeline's clock stays real and has no seam to pin anyway. Green. It preserves the only end-to-end
  proof that the BLOCK path is wired through `answer_question` (zero citations, zero contexts, prose
  withheld, real URL in the message), which an xfail would have silently discarded.
- `test_temporal_guard_blocks_the_real_dhs_fixed_admission_figure_as_of_today` keeps the real
  2026-09-15 date and **stays red on purpose**, with a docstring naming *Presidents' Alliance v. DHS*
  No. 1:26-cv-13799 (D. Mass., Saylor, J.), the 14 September order, the exact mechanism, and this
  report's section, so the next reader knows it is a live finding rather than an abandoned test. It
  carries no marker and is collected by CI's own `pytest -m "not full_corpus"` selector, verified.

`_departure_period_chunks_with_url` gained an optional `rule_effective_date` keyword defaulting to
`_FUTURE_DATE`, so all eleven existing pinned callers are byte-for-byte unaffected. **No assertion was
weakened, no threshold moved, no test skipped or xfailed, and `app/` was not touched**: the guard's
own code is unchanged, because the guard's logic is not what is broken.
...
**What could not be verified about the test split, stated rather than glossed.** No postgres is
reachable from this machine (Docker is not running), and both halves use the `pool` fixture, so
**neither test was actually executed.** What was verified: the file parses, `ruff` and `black` are
clean, both tests are collected by CI's `pytest -m "not full_corpus"` selector, and the guard verdict
each half now drives was measured directly against the real `qualify_future_dated_figures` with the
real `RetrievedChunk` shape (`_relative_future_date()` gives `blocked=True`, `_FUTURE_DATE` gives
`blocked=False` at today's date). That establishes the split does what it is meant to do at the level
of the guard. It does **not** establish that the green half's pipeline assertions still hold, since
those run through `answer_question` against a database. Expect the contract half to pass and the
real-rule half to fail; the first of those is a prediction, not a measurement, until CI runs it.
