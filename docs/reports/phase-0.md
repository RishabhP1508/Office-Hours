...
- [x] **Set a real contact in `USER_AGENT`.** Check: `.env.example`, which currently ships `contact: set-a-real-contact-here`. The builder deliberately did not put your email in a header sent to government web servers. Federal sites expect a reachable contact from a crawler; decide what you want there before crawling at any volume.
  - **Note, added 19 September 2026, historical record -- the sentence above is left as originally written.** This item was ticked satisfied by `.env.example` and the gitignored `.env` alone, and it was NOT satisfied in `app/config.py` (the `Settings.USER_AGENT` default) or in `docker-compose.yml` (which carried its own `contact: set-a-real-contact-here` placeholder) -- the two places that actually build the httpx client that reaches a government web server. Production reads neither `.env.example` nor `.env` (the latter is gitignored and not copied into the image), so it ran on `app/config.py`'s bare default, which named no contact and a repository URL that does not resolve, for the twelve days this item sat checked-as-open-but-effectively-half-done in the reader's mind. See REPORT.md instrument entry 39. Fixed the same day: `app/config.py::Settings.USER_AGENT` is now the one definition (matching what `.env.example`/`.env` always said), `docker-compose.yml` defers to it instead of restating a placeholder, and `app/config.py::require_contactable_user_agent` enforces the property at both real crawl entry points (`app/ingest.py::_ingest_from_manifest`, `app/recrawl.py::run_refresh`'s real-client branch) so this cannot silently regress to an unreachable default again.
